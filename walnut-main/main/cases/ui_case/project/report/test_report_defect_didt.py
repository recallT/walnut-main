...
@feature('项目报表-缺陷DI值每日累计趋势')
class TestPrjReportDefectCst:

    @story('T131955 缺陷DI值每日累计趋势：图表说明')
    def test_report_detail_tip(self, project_page):
        link = prj_report_link(PrjReportDefectLinks.DI_DAY_TREND, "缺陷分析",
                               "缺陷 DI 值每日累计趋势")
    ...
    @story('T131951 缺陷DI值每日累计趋势-编辑报表：y轴')
    def test_report_edit_y(self, project_page):
        link = prj_report_link(PrjReportDefectLinks.EDIT_DI_DAY_TREND, "缺陷分析",
                               "缺陷 DI 值每日累计趋势")
        project_page.driver.go_to(link)
