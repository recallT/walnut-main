...
@feature('项目集')
class TestProgramsMember:
    """项目集成员案例"""

    @story('23060 项目集成员-添加项目集成员')
    @parametrize('param', proj.add_program_user())
    def test_program_member_add(self, param, fx_program, _role_storage, _member_storage):
        """"""

        with step('添加项目集成员'):
            if _member_storage:
                param.json['item']['members'] = _member_storage[:5]
    ...
    @story('23073 成员管理-项目集下成员列表信息')
    @parametrize('param', proj.list_program_role())
    def test_program_member_list(self, param, fx_program):
        """"""
        item = pj.ItemGraphql()
    ...
    @story('23068 项目集成员-删除项目集管理员')
    @parametrize('param', proj.add_program_user())
    def test_program_member_delete(self, param, _role_storage, _member_storage, fx_program):
        """"""
    ...
    @story('23067 项目集成员-批量删除项目集成员')
    @parametrize('param', proj.add_program_user())
    def test_program_member_delete_batch(self, param, _role_storage, fx_program):
        """"""
        item = pj.ItemGraphql()
    ...
    # @story('项目集负责人信息')
    # @parametrize('param', proj.manage_program_members())
    # def test_program_member_delete(self, param, fx_program):
    #     """"""
    #     tep = pj.ItemGraphql()
    #
