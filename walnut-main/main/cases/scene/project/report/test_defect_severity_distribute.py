...
class TestDefectSeverityDistribute(Checker):

    @story(f'T151219 缺陷分析-缺陷严重程度分布报表：默认报表配置')
    def test_defect_severity_distribute_default_config(self, add_project, _report_key, _filter_config):
        ...
        with step('检查x轴配置'):
            assert config['dimensions'][0]['field_uuid'] == 'field038'
