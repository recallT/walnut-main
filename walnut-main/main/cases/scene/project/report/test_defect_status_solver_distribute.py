...
class TestDefectStatusSolverDistribute(Checker):

    @story(f'T151209 缺陷分析-缺陷状态-解决者分布报表：默认报表配置')
    def test_defect_status_solver_distribute_default_config(self, add_project, _report_key, _filter_config):
        ...
        with step('检查x轴配置'):
            assert config['dimensions'][0]['field_uuid'] == 'field005'
        ...
        with step('检查y轴配置'):
            assert config['dimensions'][1]['field_uuid'] == 'field040'
