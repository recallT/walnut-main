...
def _data():
    ...
    with step('创建一个项目'):
        time.sleep(2)
        pid = PrjAction.new_project(index=3, name='项目-工作项权限-管理所有登记工时权限')
...
@feature('项目-工作项权限-管理所有登记工时权限')
class TestPrjTaskPermAllRecordHour(Checker):

    @classmethod
    def modify_manhour(cls, tid, h_key, token=None, status_code=200):
        param = task.modify_man_hour()[0]
        param.json_update('variables.mode', 'simple')
        param.json_update('variables.task', tid)
        param.json_update('variables.type', 'recorded')
        param.json_update('variables.key', h_key)
        go(project.ItemGraphql, param, token=token, status_code=status_code)

    @story('任务-工作项权限：管理所有登记工时权限（部门）')
    def test_task_perm_department(self, _data, _department, _a_task):
        with step('清空权限域后，A编辑所有的登记工时'):
            clear_permission(permission, _data['pid'])
            self.modify_manhour(_a_task['tid'], _a_task['h_key'], token=_data['a_utoken'],
                                status_code=403)
        with step('添加权限域后，A编辑所有的登记工时'):
            add_permission(_data['pid'], _data['it_id'], permission,
                           "department", _department)
            self.modify_manhour(_a_task['tid'], _a_task['h_key'], token=_data['a_utoken'],
                                status_code=200)

    @story('任务-工作项权限：管理所有登记工时权限（成员）')
    ...
    @story('任务-工作项权限：管理所有登记工时权限（工作项创建者）')
    ...
    @story('任务-工作项权限：管理所有登记工时权限（工作项负责人）')
    ...
    @story('任务-工作项权限：管理所有登记工时权限（工作项关注者）')
    ...
    @story('任务-工作项权限：管理所所有登记工时权限（项目负责人）')
    ...
    @story('任务-工作项权限：管理所有登记工时权限（项目角色）')
    ...
    @story('T133003 任务-工作项权限：管理所有登记工时权限（自定义单选成员属性）')
    ...
    @story('T133004 任务-工作项权限：管理所有登记工时权限（自定义多选成员属性）')
    def test_task_perm_custom_field_type_13(self, _data, _a_task):
        with step('清空限域后，A编辑所有的登记工时'):
            clear_permission(permission, _data['pid'])
            self.modify_manhour(_a_task['tid'], _a_task['h_key'], token=_data['a_utoken'],
                                status_code=403)
        with step('添加权限域后，A编辑所有的登记工时'):
    ...
    @story('T142165 子任务-工作项权限：管理所有登记工时权限（部门）')
    ...
    @story('T142172 子任务-工作项权限：管理所有登记工时权限（用户组）')
    ...
    @story('T142167 子任务-工作项权限：管理所有登记工时权限（工作项创建者）')
    ...
    @story('T142168 子任务-工作项权限：管理所有登记工时权限（工作项负责人）')
    ...
    @story('T142169 子任务-工作项权限：管理所有登记工时权限（工作项关注者）')
    ...
    @story('子任务-工作项权限：管理所有登记工时权限（项目负责人）')
    def test_sub_task_p_assign(self, _data, _a_sub_task):
        with step('项目管理员清空子任务的「管理所有登记工时」权限域'):
            clear_permission(permission, _data['pid'], it_name='子任务')
        with step('登陆成员A编辑子任务A登记工时'):
            self.modify_manhour(_a_sub_task['tid'], _a_sub_task['h_key'], token=_data['a_utoken'],
                                status_code=403)
        with step('项目管理员添加子任务的「管理所有登记工时」权限域为（项目负责人）'):
            add_permission(_data['pid'], _data['s_it_id'], permission, "project_assign", "")
        with step('成员A编辑子任务A的登记工时记录'):
            self.modify_manhour(_a_sub_task['tid'], _a_sub_task['h_key'], token=_data['a_utoken'],
                                status_code=200)

    @story('T142171 子任务-工作项权限：管理所有登记工时权限（项目角色）')
    ...
    @story('T142173 子任务-工作项权限：管理所有登记工时权限（自定义单选成员属性')
    ...
    @story('T142174 子任务-工作项权限：管理所有登记工时权限（自定义多选成员属性）')
