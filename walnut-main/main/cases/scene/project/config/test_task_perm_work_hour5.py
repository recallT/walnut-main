...
def _data():
    ...
    with step('创建一个项目'):
        time.sleep(2)
        pid = PrjAction.new_project(index=3, name='项目-工作项权限-管理自己登记工时权限')
    ...
    with step('清空任务、子任务的权限域：管理所有登记工时'):
        clear_permission('manage_task_record_manhours', pid, it_name='任务')
        clear_permission('manage_task_record_manhours', pid, it_name='子任务')
    more_fields = []
...
@feature('项目-工作项权限-管理自己登记工时权限')
class TestPrjTaskPermOOwnRecordHour(Checker):

    @classmethod
    def modify_manhour(cls, tid, h_key, owner, token=None, status_code=200):
    ...
    @story('任务-工作项权限：管理自己登记工时权限（工作项创建者）')
    ...
    @story('任务-工作项权限：管理自己登记工时权限（工作项负责人）')
    ...
    @story('任务-工作项权限：管理自己登记工时权限（工作项关注者）')
    ...
    @story('任务-工作项权限：管理所自己登记工时权限（项目负责人）')
    ...
    @story('任务-工作项权限：管理自己登记工时权限（项目角色）')
    ...
    @story('T133043 任务-工作项权限：管理自己登记工时权限（自定义单选成员属性）')
    ...
    @story('T133044 任务-工作项权限：管理自己登记工时权限（自定义多选成员属性）')
    def test_task_perm_custom_field_type_13(self, _data, _a_task):
        with step('清空限域后，A编辑自己的登记工时'):
            clear_permission(permission, _data['pid'])
            self.modify_manhour(_a_task['tid'], _a_task['h_key'], _data['a_uid'], token=_data['a_utoken'],
                                status_code=403)
        with step('添加权限域后，A编辑自己的登记工时'):
    ...
    @story('T142205 子任务-工作项权限：管理自己登记工时权限（部门）')
    ...
    @story('T142212 子任务-工作项权限：管理自己登记工时权限（用户组）')
    ...
    @story('T142207 子任务-工作项权限：管理自己登记工时权限（工作项创建者）')
    ...
    @story('T142208 子任务-工作项权限：管理自己登记工时权限（工作项负责人）')
    ...
    @story('T142209 子任务-工作项权限：管理自己登记工时权限（工作项关注者）')
    ...
    @story('子任务-工作项权限：管理自己登记工时权限（项目负责人）')
    ...
    @story('T142213 子任务-工作项权限：管理自己登记工时权限（自定义单选成员属性')
    ...
    @story('T142214 子任务-工作项权限：管理自己登记工时权限（自定义多选成员属性）')
