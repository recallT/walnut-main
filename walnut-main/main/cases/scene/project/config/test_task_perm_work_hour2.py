...
def _data():
    ...
    with step('创建一个项目'):
        time.sleep(2)
        pid = PrjAction.new_project(index=3, name='项目-工作项权限-管理所有预估工时权限')
...
def _a_task(_data):
    ...
    with step('管理员-新增1条预估工时'):
        t_h_key = add_manhour(tid)
...
def _a_sub_task(_data, _a_task):
    ...
    with step('管理员-新增一条工时记录'):
        s_t_h_key = add_manhour(s_tid)
...
@feature('项目-工作项权限-管理所有预估工时权限')
class TestPrjTaskPermAllAssessHour(Checker):

    @classmethod
    def modify_manhour(cls, tid, t_h_key, token=None, status_code=200):
    ...
    @story('T133008 任务-工作项权限：管理所有预估工时权限（部门）')
    def test_task_perm_department(self, _data, _department, _a_task):
        ...
        with step('登陆成员A编辑任务A预估工时'):
            self.modify_manhour(_a_task['tid'], _a_task['h_key'], token=_data['a_utoken'], status_code=403)
        with step('项目管理员添加任务的「管理所有预估工时」权限域为部门1'):
            add_permission(_data['pid'], _data['it_id'], permission,
                           "department", _department)
        with step('成员A编辑任务A的预估工时记录'):
            self.modify_manhour(_a_task['tid'], _a_task['h_key'], token=_data['a_utoken'], status_code=200)

    @story('T133015 任务-工作项权限：管理所有预估工时权限（用户组）')
    def test_task_perm_group(self, _data, _group, _a_task):
        with step('项目管理员清空任务的「管理所有预估工时」权限域'):
            clear_permission(permission, _data['pid'])
        with step('登陆成员A编辑任务A预估工时'):
            self.modify_manhour(_a_task['tid'], _a_task['h_key'], token=_data['a_utoken'], status_code=403)
        with step('项目管理员添加任务的「管理所有预估工时」权限域为用户组1'):
            add_permission(_data['pid'], _data['it_id'], permission,
                           "group", _group)
        with step('成员A编辑任务A的预估工时记录'):
            self.modify_manhour(_a_task['tid'], _a_task['h_key'], token=_data['a_utoken'], status_code=200)
    ...
    def test_task_t_creator(self, _data, _a_task):
        with step('项目管理员清空任务的「管理所有预估工时」权限域'):
            clear_permission(permission, _data['pid'])
        with step('登陆成员A编辑任务A预估工时'):
            self.modify_manhour(_a_task['tid'], _a_task['h_key'], token=_data['a_utoken'],
                                status_code=403)
        with step('项目管理员添加任务的「管理所有预估工时」权限域为（工作项创建者）'):
            add_permission(_data['pid'], _data['it_id'], permission, "task_owner", "")
        with step('成员A编辑任务A的预估工时记录'):
            self.modify_manhour(_a_task['tid'], _a_task['h_key'], token=_data['a_utoken'],
                                status_code=200)

    @story('T133011 任务-工作项权限：管理所有预估工时权限（工作项负责人）')
    def test_task_t_assigner(self, _data, _a_task):
        with step('项目管理员清空任务的「管理所有预估工时」权限域'):
            clear_permission(permission, _data['pid'])
        with step('登陆成员A编辑任务A预估工时'):
            self.modify_manhour(_a_task['tid'], _a_task['h_key'], token=_data['a_utoken'],
                                status_code=403)
        with step('项目管理员添加任务的「管理所有预估工时」权限域为（工作项负责人）'):
            add_permission(_data['pid'], _data['it_id'], permission, "task_assign", "")
        with step('成员A编辑任务A的预估工时记录'):
            self.modify_manhour(_a_task['tid'], _a_task['h_key'], token=_data['a_utoken'],
                                status_code=200)

    @story('T133012 任务-工作项权限：管理所有预估工时权限（工作项关注者）')
    def test_task_t_watcher(self, _data, _a_task):
        with step('项目管理员清空任务的「管理所有预估工时」权限域'):
            clear_permission(permission, _data['pid'])
        with step('登陆成员A编辑任务A预估工时'):
            self.modify_manhour(_a_task['tid'], _a_task['h_key'], token=_data['a_utoken'],
                                status_code=403)
        with step('项目管理员添加任务的「管理所有预估工时」权限域为（工作项关注者）'):
            add_permission(_data['pid'], _data['it_id'], permission, "task_watchers", "")
        with step('成员A编辑任务A的预估工时记录'):
            self.modify_manhour(_a_task['tid'], _a_task['h_key'], token=_data['a_utoken'],
                                status_code=200)

    @story('T133013 任务-工作项权限：管理所有预估工时权限（项目负责人）')
    def test_task_p_assign(self, _data, _a_task):
        with step('项目管理员清空任务的「管理所有预估工时」权限域'):
            clear_permission(permission, _data['pid'])
        with step('登陆成员A编辑任务A预估工时'):
            self.modify_manhour(_a_task['tid'], _a_task['h_key'], token=_data['a_utoken'],
                                status_code=403)
        with step('项目管理员添加任务的「管理所有预估工时」权限域为（项目负责人）'):
            add_permission(_data['pid'], _data['it_id'], permission, "project_assign", "")
        with step('成员A编辑任务A的预估工时记录'):
            self.modify_manhour(_a_task['tid'], _a_task['h_key'], token=_data['a_utoken'],
                                status_code=200)

    @story('T133014 任务-工作项权限：管理所有预估工时权限（项目角色）')
    def test_task_p_role(self, _data, _a_task):
        with step('项目管理员清空任务的「管理所有预估工时」权限域'):
            clear_permission(permission, _data['pid'])
        with step('登陆成员A编辑任务A预估工时'):
            self.modify_manhour(_a_task['tid'], _a_task['h_key'], token=_data['a_utoken'],
                                status_code=403)
        with step('项目管理员添加任务的「管理所有预估工时」权限域为（项目角色-项目成员）'):
    ...
    @story('T133016 任务-工作项权限：管理所有预估工时权限（自定义单选成员属性）')
    def test_task_perm_custom_field_type_8(self, _data, _a_task):
        with step('项目管理员清空任务的「管理所有预估工时」权限域'):
            clear_permission(permission, _data['pid'])
        with step('登陆成员A编辑任务A预估工时'):
            self.modify_manhour(_a_task['tid'], _a_task['h_key'], token=_data['a_utoken'], status_code=403)
        with step('项目管理员添加任务的「管理所有预估工时」权限域为示例单选成员'):
            add_permission(_data['pid'], _data['it_id'], permission,
                           "custom_member_fields", _data['fid_map']['自定义单选成员'])
        with step('成员A编辑任务A的预估工时记录'):
            self.modify_manhour(_a_task['tid'], _a_task['h_key'], token=_data['a_utoken'], status_code=200)

    @story('T133017 任务-工作项权限：管理所有预估工时权限（自定义多选成员属性）')
    def test_task_perm_custom_field_type_13(self, _data, _a_task):
        with step('项目管理员清空任务的「管理所有预估工时」权限域'):
            clear_permission(permission, _data['pid'])
        with step('登陆成员A编辑任务A预估工时'):
            self.modify_manhour(_a_task['tid'], _a_task['h_key'], token=_data['a_utoken'], status_code=403)
        with step('项目管理员添加任务的「管理所有预估工时」权限域为示例多选成员'):
            add_permission(_data['pid'], _data['it_id'], permission,
                           "custom_member_fields", _data['fid_map']['自定义多选成员'])
        with step('成员A编辑任务A的预估工时记录'):
            self.modify_manhour(_a_task['tid'], _a_task['h_key'], token=_data['a_utoken'], status_code=200)

    @story('T142178 子任务-工作项权限：管理所有预估工时权限（部门）')
    def test_sub_task_perm_department(self, _data, _department, _a_sub_task):
        with step('项目管理员清空子任务的「管理所有预估工时」权限域'):
            clear_permission(permission, _data['pid'], it_name='子任务')
        with step('登陆成员A编辑子任务A预估工时'):
            self.modify_manhour(_a_sub_task['tid'], _a_sub_task['h_key'], token=_data['a_utoken'], status_code=403)
        with step('项目管理员添加子任务的「管理所有预估工时」权限域为部门1'):
            add_permission(_data['pid'], _data['s_it_id'], permission,
                           "department", _department)
        with step('成员A编辑子任务A的预估工时记录'):
            self.modify_manhour(_a_sub_task['tid'], _a_sub_task['h_key'], token=_data['a_utoken'], status_code=200)

    @story('T142185 子任务-工作项权限：管理所有预估工时权限（用户组）')
    def test_sub_task_perm_group(self, _data, _group, _a_sub_task):
        with step('项目管理员清空子任务的「管理所有预估工时」权限域'):
            clear_permission(permission, _data['pid'], it_name='子任务')
        with step('登陆成员A编辑子任务A预估工时'):
            self.modify_manhour(_a_sub_task['tid'], _a_sub_task['h_key'], token=_data['a_utoken'], status_code=403)
        with step('项目管理员添加子任务的「管理所有预估工时」权限域为用户组1'):
            add_permission(_data['pid'], _data['s_it_id'], permission,
                           "group", _group)
        with step('成员A编辑子任务A的预估工时记录'):
            self.modify_manhour(_a_sub_task['tid'], _a_sub_task['h_key'], token=_data['a_utoken'], status_code=200)

    @story('T142180 子任务-工作项权限：管理所有预估工时权限（工作项创建者）')
    def test_sub_task_t_creator(self, _data, _a_sub_task):
        with step('项目管理员清空子任务的「管理所有预估工时」权限域'):
            clear_permission(permission, _data['pid'], it_name='子任务')
        with step('登陆成员A编辑子任务A预估工时'):
            self.modify_manhour(_a_sub_task['tid'], _a_sub_task['h_key'], token=_data['a_utoken'],
                                status_code=403)
        with step('项目管理员添加子任务的「管理所有预估工时」权限域为（工作项创建者）'):
            add_permission(_data['pid'], _data['s_it_id'], permission, "task_owner", "")
        with step('成员A编辑子任务A的预估工时记录'):
            self.modify_manhour(_a_sub_task['tid'], _a_sub_task['h_key'], token=_data['a_utoken'],
                                status_code=200)

    @story('T142181 子任务-工作项权限：管理所有预估工时权限（工作项负责人）')
    def test_sub_task_t_assigner(self, _data, _a_sub_task):
        ...
        with step('登陆成员A编辑子任务A预估工时'):
            self.modify_manhour(_a_sub_task['tid'], _a_sub_task['h_key'], token=_data['a_utoken'],
                                status_code=403)
        with step('项目管理员添加子任务的「管理所有预估工时」权限域为（工作项负责人）'):
            add_permission(_data['pid'], _data['s_it_id'], permission, "task_assign", "")
        with step('成员A编辑子任务A的预估工时记录'):
            self.modify_manhour(_a_sub_task['tid'], _a_sub_task['h_key'], token=_data['a_utoken'],
                                status_code=200)

    @story('T142182 子任务-工作项权限：管理所有预估工时权限（工作项关注者）')
    def test_sub_task_t_watcher(self, _data, _a_sub_task):
        ...
        with step('登陆成员A编辑子任务A预估工时'):
            self.modify_manhour(_a_sub_task['tid'], _a_sub_task['h_key'], token=_data['a_utoken'],
                                status_code=403)
        with step('项目管理员添加子任务的「管理所有预估工时」权限域为（工作项关注者）'):
            add_permission(_data['pid'], _data['s_it_id'], permission, "task_watchers", "")
        with step('成员A编辑子任务A的预估工时记录'):
            self.modify_manhour(_a_sub_task['tid'], _a_sub_task['h_key'], token=_data['a_utoken'],
                                status_code=200)

    @story('T142183 子任务-工作项权限：管理所有预估工时权限（项目负责人）')
    def test_sub_task_p_assign(self, _data, _a_sub_task):
        ...
        with step('登陆成员A编辑子任务A预估工时'):
            self.modify_manhour(_a_sub_task['tid'], _a_sub_task['h_key'], token=_data['a_utoken'],
                                status_code=403)
        with step('项目管理员添加子任务的「管理所有预估工时」权限域为（项目负责人）'):
            add_permission(_data['pid'], _data['s_it_id'], permission, "project_assign", "")
        with step('成员A编辑子任务A的预估工时记录'):
            self.modify_manhour(_a_sub_task['tid'], _a_sub_task['h_key'], token=_data['a_utoken'],
                                status_code=200)

    @story('T142184 子任务-工作项权限：管理所有预估工时权限（项目角色）')
    def test_sub_task_p_role(self, _data, _a_sub_task):
        ...
        with step('登陆成员A编辑子任务A预估工时'):
            self.modify_manhour(_a_sub_task['tid'], _a_sub_task['h_key'], token=_data['a_utoken'],
                                status_code=403)
        with step('项目管理员添加子任务的「管理所有预估工时」权限域为（项目角色-项目成员）'):
            role_id = get_default_role(_data['pid'])
    ...
    @story('T142186 子任务-工作项权限：管理所有预估工时权限（自定义单选成员属性')
    def test_sub_task_perm_custom_field_type_8(self, _data, _a_sub_task):
        with step('项目管理员清空子任务的「管理所有预估工时」权限域'):
            clear_permission(permission, _data['pid'], it_name='子任务')
        with step('登陆成员A编辑子任务A预估工时'):
            self.modify_manhour(_a_sub_task['tid'], _a_sub_task['h_key'], token=_data['a_utoken'], status_code=403)
        with step('项目管理员添加子任务的「管理所有预估工时」权限域为示例单选成员'):
            add_permission(_data['pid'], _data['s_it_id'], permission,
                           "custom_member_fields", _data['fid_map']['自定义单选成员'])
        with step('成员A编辑子任务A的预估工时记录'):
            self.modify_manhour(_a_sub_task['tid'], _a_sub_task['h_key'], token=_data['a_utoken'], status_code=200)

    @story('T142187 子任务-工作项权限：管理所有预估工时权限（自定义多选成员属性）')
    def test_sub_task_perm_custom_field_type_13(self, _data, _a_sub_task):
        with step('项目管理员清空子任务的「管理所有预估工时」权限域'):
            clear_permission(permission, _data['pid'], it_name='子任务')
        with step('登陆成员A编辑子任务A预估工时'):
            self.modify_manhour(_a_sub_task['tid'], _a_sub_task['h_key'], token=_data['a_utoken'], status_code=403)
        with step('项目管理员添加子任务的「管理所有预估工时」权限域为示例多选成员'):
            add_permission(_data['pid'], _data['s_it_id'], permission,
                           "custom_member_fields", _data['fid_map']['自定义多选成员'])
        with step('成员A编辑子任务A的预估工时记录'):
            self.modify_manhour(_a_sub_task['tid'], _a_sub_task['h_key'], token=_data['a_utoken'], status_code=200)
